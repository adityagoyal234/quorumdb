...
func startCluster(t *testing.T) func() {
	...
	dirs := []string{
		"/tmp/quorumdb-test/node1",
		"/tmp/quorumdb-test/node2",
		"/tmp/quorumdb-test/node3",
	}
	...
	cmds := []*exec.Cmd{
		exec.Command("go", "run", "../../cmd/quorumdb/main.go",
			"--node-id=node1", "--port=8001", "--gossip-port=7001", "--data-dir=/tmp/quorumdb-test/node1"),
		exec.Command("go", "run", "../../cmd/quorumdb/main.go",
			"--node-id=node2", "--port=8002", "--gossip-port=7002", "--data-dir=/tmp/quorumdb-test/node2",
			"--seeds=127.0.0.1:7001"),
		exec.Command("go", "run", "../../cmd/quorumdb/main.go",
			"--node-id=node3", "--port=8003", "--gossip-port=7003", "--data-dir=/tmp/quorumdb-test/node3",
	...
	return func() {
		...
		os.RemoveAll("/tmp/quorumdb-test")
	}
}
