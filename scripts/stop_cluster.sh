...
# Stop QuorumDB cluster script

DATA_DIR="/tmp/quorumdb-cluster"
PIDS_FILE="$DATA_DIR/pids.txt"

echo "Stopping QuorumDB cluster..."
...
if [ -f "$PIDS_FILE" ]; then
    ...
    # Find and kill any quorumdb processes
    pkill -f "bin/quorumdb" 2>/dev/null || true
fi
